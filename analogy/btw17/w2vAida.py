...
class Word2Vec(utils.SaveLoad):
    ...
    def save_word2vec_format_new(self, fname, fvocab=None, binary=False):
        ...
        if fvocab is not None:
            ...
            with utils.smart_open(fvocab, 'wb') as vout:
                for word, vocab in sorted(iteritems(self.vocab), key=lambda item: -item[1].count):
                    array=word.split(multi_vector_delimeter)
                    #print array
                    vout.write(utils.to_utf8("%s %s\n" % (array[0], vocab)))   # potential BUG here: should this be vocab.count ??
        logger.info("storing %sx%s projection weights into %s" % (len(self.vocab), self.layer1_size, fname))
        assert (len(self.vocab), self.layer1_size) == self.syn0.shape
        with utils.smart_open(fname, 'wb') as fout:
            fout.write(utils.to_utf8("%s %s\n" % self.syn0.shape))
    ...
    @classmethod
    def load_word2vec_format(self, fname, fvocab=None, binary=False, norm_only=True, delimeter=':'):
    ...
    def similar_word(self, word):
        res = []
        for w in self.vocab:
            #w1=re.split(multi_vector_delimeter,w)
